...
def write_file(file_name,content_towrite,dpu_name):
    ...
    try:
        ...
        logger.log_error(f'{dpu_name}:Failed to write {content_towrite} to file {file_name}')
...
class DpuCtlPlat():
    ...
    def __init__(self, dpu_index):
        ...
        self.set_go_down_path = os.path.join(SYSTEM_BASE, f"dpu{self.index}_rst")
        self.set_pwr_path = os.path.join(SYSTEM_BASE, f"dpu{self.index}_pwr")
        self.set_pwr_f_path = os.path.join(SYSTEM_BASE, f"dpu{self.index}_pwr_force")
        self.get_dpu_rdy_path = os.path.join(EVENT_BASE, f"dpu{self.index}_ready")
        self.set_dpu_perst_en_path = os.path.join(SYSTEM_BASE, f"dpu{self.index}_perst_en")
    ...
    def dpu_go_down(self):
        ...
        get_shtdn_ready_path = os.path.join(EVENT_BASE, f"dpu{self.index}_shtdn_ready")
        try:
    ...
    def dpu_burn_fw(self,path):
        ...
        """if not os.path.isfile(path):
            raise FileNotFoundError(f"{self.get_name()}:File {self.file_name} does not exist!")
        cmd = ["sonic_bfb_install" ,"-b" ,path,"-r","rshim"+str(self.index)]
        try:
            cmd_output = subprocess.check_output(cmd)
        except subprocess.CalledProcessError as cmd_exc:
            print("Installation failed! code", cmd_exc.returncode, cmd_exc.output)"""
    ...
    def dpu_pci_remove(self):
        ...
        get_dpu_pci_path = os.path.join(CONFIG_BASE, f"dpu{self.index}_pci_bus_id")
        pci_string = utils.read_str_from_file(get_dpu_pci_path,raise_exception=True)
