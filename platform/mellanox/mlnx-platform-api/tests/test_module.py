...
class TestModule:
    ...
    def test_module_vpd(self):
        ...
        assert m.get_serial() == 'MT1946X07684'
        assert m.get_revision() == 'A3'
        with patch("swsscommon.swsscommon.SonicV2Connector.__init__", mock.MagicMock(return_value=None)), \
         patch("swsscommon.swsscommon.SonicV2Connector.connect", mock.MagicMock(return_value=None)):
    ...
    def test_dpu_module(self, mock_call, mock_get):
        ...
        m1.midplane_ip = None
        m.midplane_ip = None
        assert m.get_midplane_ip() == "169.254.200.244"
        assert m1.get_midplane_ip() == "169.254.200.243"
        ...
        with patch("sonic_platform.utils.read_int_from_file", wraps=mock_read_int_from_file):
            ...
            assert m.get_reboot_cause() == (ChassisBase.REBOOT_CAUSE_HARDWARE_OTHER, 'Reset by the TPM module')
            test_file_path = "perst_rst"
            assert m.get_reboot_cause() == (ChassisBase.REBOOT_CAUSE_HARDWARE_OTHER, 'PERST# signal to ASIC')
            test_file_path = "phy_rst"
            assert m.get_reboot_cause() == (ChassisBase.REBOOT_CAUSE_HARDWARE_OTHER, 'Phy reset')
            test_file_path = "usbphy_rst"
            assert m.get_reboot_cause() == (ChassisBase.REBOOT_CAUSE_HARDWARE_OTHER, 'USB Phy reset')
            test_file_path = "None"
            assert m.get_reboot_cause() == (ChassisBase.REBOOT_CAUSE_HARDWARE_OTHER, '')
        appl_db_data = {
