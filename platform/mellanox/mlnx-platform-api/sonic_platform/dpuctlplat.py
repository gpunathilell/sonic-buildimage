...
class DpuCtlPlat():
    ...
    def __init__(self, dpu_name):
        ...
        self.get_shtdn_ready_path = os.path.join(SYSTEM_BASE,
    ...
    def write_file(self, file_name, content_towrite):
        ...
        try:
            ...
            raise type(e)(f"{self.get_name()}:{str(e)}")
    ...
    def dpu_go_down(self):
        ...
        if not dpu_shtdn_rdy:
            logger.log_error(f"{self.get_name()}: Going Down Unsuccessful")
            return False
        return True
    ...
    def _power_on_force(self, count=4):
        ...
        if not dpu_rdy:
            ...
            logger.log_error(f"{self.get_name()}: Failed Force power on! Exiting")
            return False
