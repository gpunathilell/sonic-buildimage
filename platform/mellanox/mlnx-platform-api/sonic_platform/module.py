...
class DpuModule(ModuleBase):
    ...
    def get_dpu_id(self):
        ...
        Returns:
            An integer, indicating the DPU ID. DPU0 returns 0, DPUX returns X+1
    ...
    def get_reboot_cause(self):
        ...
        self.reboot_cause_map = {
            f'/run/hw-management/system/dpu{self._name}/system/reset_aux_pwr_or_reload':
                (ChassisBase.REBOOT_CAUSE_POWER_LOSS, ''),
            f'/run/hw-management/system/dpu{self._name}/system/tpm_rst':
                (ChassisBase.REBOOT_CAUSE_HARDWARE_OTHER, 'Reset by the TPM module'),
            f'/run/hw-management/system/dpu{self._name}/system/perst_rst':
                (ChassisBase.REBOOT_CAUSE_HARDWARE_OTHER, 'PERST# signal to ASIC'),
            f'/run/hw-management/system/dpu{self._name}/system/phy_rst':
                (ChassisBase.REBOOT_CAUSE_HARDWARE_OTHER, 'Phy reset'),
            f'/run/hw-management/system/dpu{self._name}/system/usbphy_rst':
    ...
    def get_midplane_ip(self):
        ...
        if not self.midplane_ip:
            ...
            self.midplane_ip = str(ip_network_cidr[self.dpu_id+1])
